...
import { checkTwoFactorRateLimit, finalizeTwoFactorVerification } from "@/actions/two-factor"
...
export function TwoFactorVerifyForm() {
    ...
    const mapClientError = (error: { code?: string; message?: string } | null): string => {
        switch (error?.code) {
            case "INVALID_TWO_FACTOR_AUTHENTICATION": return "Invalid verification code"
    ...
    const verifyCode = async (codeToVerify: string) => {
        ...
        // Phase 1: server-side rate limit + format validation
        const preCheck = await checkTwoFactorRateLimit(codeToVerify, inputMode)
        if (preCheck.error) {
            showError(preCheck.error)
            return
        }

        // Phase 2: client-side HTTP call so better-auth can set cookies properly
        const result = inputMode === "totp"
            ? await authClient.twoFactor.verifyTotp({ code: codeToVerify })
            : await authClient.twoFactor.verifyBackupCode({ code: codeToVerify })

        if (result.error) {
            showError(mapClientError(result.error))
            return
        }

        // Mark session as 2FA-verified in DB and clear cookie cache.
        // better-auth's verify-totp doesn't update twoFactorVerified on existing
        // sessions (its hook only covers email/password sign-in).
        await finalizeTwoFactorVerification()
