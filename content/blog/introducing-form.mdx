...
Today we're launching **anon.li Form** — the third pillar alongside Alias and Drop. Build a form, share the link, and read submissions that only you can decrypt. Our server stores response content as ciphertext; only operational metadata remains readable.

## How it works

When you create a form, your browser generates a fresh P-256 keypair. The public key ships with the form so any visitor can encrypt to it. The private key is wrapped under your account vault key — the same vault that protects your Drop owner keys — and stored server-side in that wrapped form. We never see it in the clear.

When someone submits, their browser:

1. Generates an ephemeral P-256 keypair for that submission.
...
- **Password protection.** Add a passphrase and the form withholds its questions from anyone who can't supply it. Argon2id runs in the browser and only a password-derived witness is sent for verification, so the password never reaches our server.
...
- **Turnstile.** Public submissions are gated by Cloudflare Turnstile, even when the respondent happens to have an anon.li session. A respondent's login never becomes an abuse bypass against somebody else's form.
...
- **Plus ($3.99/mo)** — 10 forms, 1,000 submissions/month, 90-day retention, password protection, 5 GB attachments.
- **Pro ($5.99/mo)** — 30 forms, 10,000 submissions/month, 365-day retention, branding removal, 50 GB attachments.
...
- **Can see:** the form's title, description, and field schema at rest, submission timestamps, submission counts, the IP hash of the submitter (peppered, for abuse-rate-limiting), and the form owner's account. A password-protected form's schema is not served until the visitor proves knowledge of its password.
...
If you're a developer, the [Form API reference](/docs/api/form) covers the endpoints, the encryption layout, and the schema. The whole platform is [open source](https://github.com/anondotli/anon.li) — every claim above is verifiable in `lib/services/form.ts` and the public form components.
