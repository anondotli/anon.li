...
The IV handling is where a lot of AES-GCM deployments go wrong, so it's worth being specific. Each Drop gets a random 12-byte metadata IV, and every file gets its own independent random 12-byte base IV. For each file chunk we derive a nonce by taking the first 8 bytes of that file IV and appending the 32-bit chunk index in big-endian. Filenames and titles use the reserved domain `0xFFFFFFFF`; messages use `0xFFFFFFFE`. The domains cannot collide with data chunks, and title/message encryption cannot reuse a nonce. IV reuse in GCM is catastrophic; these independent bases and explicit domains keep every use under a Drop key separate.
