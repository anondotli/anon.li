...
Our full source code is available on [GitHub](https://github.com/anondotli). We use only standard, well-audited cryptographic primitives - no custom cryptography.
...
The server can never derive or recover this key on its own. The one documented exception is when someone who already holds the link voluntarily submits the key — for example, an abuse reporter pasting a full Drop URL into a report. In that case the submitted key is stored encrypted at rest (see Report Encryption, below), so reported content is protected even against a database compromise.

### IV and Chunk Encryption

- **Base IV**: 12 bytes of cryptographically random data, generated per file
- **Chunk IVs**: Derived deterministically from the base IV - the first 8 bytes of the base IV form a 64-bit random nonce prefix, and the chunk index is appended as a 4-byte big-endian counter. This gives every chunk within a file a unique IV without additional random generation. Because each file is encrypted under a fresh, random 256-bit key, the (key, IV) pair is unique for every chunk, which is the property AES-GCM requires
- **Chunk size**: Approximately 50 MiB of plaintext per part (the final part may be smaller), up to the S3/R2 limit of 10,000 parts per file. A 250 GiB file uses 5,120 bounded-size parts rather than multi-gigabyte browser buffers
...
Filenames and drop titles use the reserved domain `0xFFFFFFFF`; drop messages use the distinct domain `0xFFFFFFFE`. File contents only use chunk indexes starting at zero. Together with an independent random base IV for each file and for drop metadata, those domains prevent AES-GCM nonce reuse. The server stores only encrypted filenames and metadata, so it cannot determine what you are sharing.
...
Connections to the hosted service use TLS. Encrypted file data is stored in a private Cloudflare R2 bucket and served directly to clients through the R2 S3 API endpoint, so blob traffic bypasses our Next.js servers entirely. Downloads use short-lived presigned URLs. Multipart upload URLs are bound to an upload ID, part number, and exact encrypted byte length, and may remain valid for up to seven days so a 250 GiB transfer can finish on an ordinary connection. Public bucket URLs and custom domains are disabled so application expiry, revocation, recipient, and download-limit checks cannot be bypassed.
...
| Account data | Aliases, domains, files, drops, API keys, recipients, sign-in credentials, vault materials, and the account record are deleted from live systems immediately on account deletion |
| IP addresses | Never stored in sessions; hashed for abuse reports only; ephemeral in rate-limit cache |
| Analytics | [PostHog](https://posthog.com/) (EU region), cookie-free (local-storage id), no session replay; URL fragments and resource IDs are stripped before any event is sent; not loaded on Drop download, auth/verify/reset, or admin routes |
...
The anon.li codebase is open source on [GitHub](https://github.com/anondotli):

- [**Web application**](https://github.com/anondotli/anon.li) - Next.js, Prisma, TypeScript
- [**Mail server (MX)**](https://github.com/anondotli/mx) - Haraka-based forwarding server
- [**CLI**](https://github.com/anondotli/cli) - Command-line client for Alias and Drop
- [**Browser extension**](https://github.com/anondotli/extension) - Firefox & Chrome browser extension
