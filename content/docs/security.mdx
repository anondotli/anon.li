...
Users can upload an OpenPGP public key in their settings. When enabled, the forwarded copy of each email is encrypted with the user's public key before delivery using [OpenPGP.js](https://openpgpjs.org/). This protects the delivered message in the destination mailbox and limits exposure after forwarding, but standard alias forwarding still requires transient server-side processing of the original message. Public key fingerprints are cached using SHA-256 hashes for efficient lookup.
...
anon.li uses magic link authentication - there is no password database to breach. Users receive a one-time login link via email, eliminating the risk of credential stuffing, password reuse, and brute-force attacks.
...
- **Enforcement**: 2FA is checked on every authenticated request (both session-based and API). Sessions that require 2FA but haven't completed verification are rejected with 401
...
All endpoints are protected by sliding-window rate limiting via [Upstash Redis](https://upstash.com/). Limits are tuned per-endpoint based on expected usage patterns. Examples:
...
| File upload (authenticated) | 100 | 1 hour |
| File upload (guest) | 50 | 1 hour |
| Drop creation | 20 | 1 hour |
| Abuse reports | 5 | 1 hour |

Paid plans include monthly API quota limits enforced at the account level.
...
Unauthenticated actions (registration, abuse reports, guest uploads) are protected by [Cloudflare Turnstile](https://www.cloudflare.com/products/turnstile/) CAPTCHA verification when enabled in production. Turnstile is enforced in the production deployment but is configurable via environment variables for development and self-hosted instances.
...
Anyone can submit an abuse report with Cloudflare Turnstile verification. Reports are deduplicated by IP hash and resource ID within a 24-hour window to prevent spam. Each report receives a tracking token for anonymous status checks - reporters don't need an account to follow up.
...
| Account data | Aliases, domains, files, drops, API keys, and recipients deleted immediately on account deletion. The user record itself is hard-deleted after 30 days |
| IP addresses | Never stored in sessions; hashed for abuse reports only; ephemeral in rate-limit cache |
| Analytics | [Umami](https://umami.is/) - cookie-free, no PII collected |
