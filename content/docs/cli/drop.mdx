...
3. Each file is split into 50 MB chunks, each encrypted with an IV derived from the base IV + chunk index
4. Encrypted chunks are uploaded concurrently to storage
5. The share URL is returned with the decryption key in the fragment: `https://anon.li/d/<id>#<key>`
