...
This Privacy Policy explains how **anon.li** ("we," "our," or "us") collects, uses, and protects your information when you use our website `https://anon.li` and our associated services (collectively, the "Service").
...
- **Analytics & product monitoring**: We use [PostHog](https://posthog.com/) (EU region) to understand aggregate usage and improve the product. PostHog does **not** use cookies (a pseudonymous identifier is stored in your browser's local storage), session replay is **disabled**, and we honor the browser's **Do Not Track** signal. Before any event leaves your browser we strip URL fragments — which carry Drop decryption keys — and filter out resource identifiers. Analytics events are discarded on public Drop and Form links, password-reset, recipient-verification, and 2FA pages. After you sign in, events are linked to your account by an internal id only — never your email.
...
| **PostHog** | Product analytics & error monitoring | Pseudonymous usage events & sanitized error reports (no cookies, no session replay, no Drop keys/PII) | EU |
...
We do not track users across third-party websites and do not engage in behavioral tracking or targeted advertising. Our analytics provider (PostHog, EU region) does not use cookies, and analytics honor your browser's Do Not Track setting.
