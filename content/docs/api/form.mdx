...
The top-level `title` (required, 1–300 chars) and `description` (optional, ≤ 2000 chars) mirror the form's top-level metadata and are kept in sync server-side. `title` and `description` may also be passed separately when creating/updating a form; the stored schema is always reconciled to match.
