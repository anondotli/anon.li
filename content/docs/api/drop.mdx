...
The Drop API supports three authentication modes depending on the endpoint and use case:
...
3. **Anonymous / Guest** - creating a drop (`POST /drop`) and uploading files does not require authentication. A Turnstile CAPTCHA token is required for guest uploads. Downloading a drop's files is always public for anyone with the drop ID.
...
```json
{
  ...
  "uploadUrls": [
    "https://presigned-url-for-chunk-1...",
    "https://presigned-url-for-chunk-2..."
  ]
}
```
...
const file = await fetch(`https://anon.li/api/v1/drop/${drop.dropId}/file`, {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer ak_...',
    'Content-Type': 'application/json'
...
await fetch(file.uploadUrls[0], {
  method: 'PUT',
  body: encryptedData
});

// 7. Complete the drop (marks it ready for download)
await fetch(`https://anon.li/api/v1/drop/${drop.dropId}?action=complete`, {
  method: 'PATCH',
  headers: { 'Authorization': 'Bearer ak_...' }
});

// 8. Build share URL with key in fragment
const shareUrl = `https://anon.li/d/${drop.dropId}#${key.toString('base64url')}`;
...
| Operation | Limit |
| :--- | :--- |
| Drop creation | 30 requests/hour |
| Drop list | 60 requests/minute |
| Drop delete | 60 requests/hour |
