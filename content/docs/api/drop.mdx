...
<div className="flex items-center gap-2 mt-6 mb-4">
    ...
    <code className="text-sm bg-secondary px-2 py-1 rounded">https://anon.li/api/v1/vault/unlock</code>
</div>
...
| Field | Type | Required | Description |
| :--- | :--- | :--- | :--- |
| `size` | number | Yes | Encrypted file size in bytes, including one 16-byte AES-GCM tag per part |
| `encryptedName` | string | Yes | Encrypted filename (base64) |
| `iv` | string | Yes | File IV (base64, 12 bytes) |
| `mimeType` | string | Yes | MIME type |
| `chunkCount` | number | Yes | Number of encrypted parts (1–10,000) |
| `chunkSize` | number | Yes | Plaintext bytes per full part; clients should target 50 MiB |
...
if (drop.encryptedMessage) {
  // Messages use a separate reserved domain from titles. Reusing a GCM nonce
  // with the same key would break confidentiality and authentication.
  const msgIv = deriveChunkIv(dropIv, 0xFFFFFFFE);
...
// Messages use their own reserved domain so a title and message never reuse
// the same AES-GCM nonce under this key.
const metaIv = deriveChunkIv(iv, 0xFFFFFFFE);
...
const metaIv = deriveChunkIv(baseIv, 0xFFFFFFFE);
const encMsg = Buffer.from(drop.encryptedMessage, 'base64url');
